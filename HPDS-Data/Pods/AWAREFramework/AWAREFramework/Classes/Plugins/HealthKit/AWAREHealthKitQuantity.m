...
@implementation AWAREHealthKitQuantity{
    ...
    NSString* KEY_VALUE;
    NSString* KEY_UNIT;
     NSString* KEY_START; //This was commented out in initial library
    NSString* KEY_END;
    NSString* KEY_DEVICE;
    NSString* KEY_LABLE;
}
...
- (instancetype)initWithAwareStudy:(AWAREStudy *)study dbType:(AwareDBType)dbType{
    ...
    if(self){
        ...
        KEY_VALUE = @"value";
        KEY_UNIT = @"unit";
        KEY_START = @"start"; //This was commented out initially
...
//    [super createTable:query]; //This was commented out in initial library
...
- (void)saveQuantityData:(NSArray *)data{
    for(HKQuantitySample *samples in data)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.storage saveDataWithDictionary:dict buffer:true saveInMainThread:true]; // Added .storage
